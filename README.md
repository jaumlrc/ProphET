...
```
carton exec ./ProphET_standalone.pl
```

When adding or removing dependencies, a new cpanfile.snapshot can be generated by running `carton install`.

## Building Docker

```
docker build -t nickp60/prophet .
```

## Running with Docker and carton

```
docker run  --rm -v ${PWD}:/input -v ${PWD}:/output nickp60/prophet carton exec /ProphET/ProphET_standalone.pl --fasta_in /input/test.fasta --gff_in /input/test.gff --outdir /output/ProphET_results
```

Here we have attached two volumes (in this case pointing to the same host directory, but could be different depending on where the input and output files are located).  We run `carton exec` before our command to ensure that the program executes with all the proper dependencies.

`-rm` makes sure we remove the container after use.

When spcifying the input and output files, ensure you put them relative to the container, not the host.  The generalization would be as follows:

```
docker run  --rm -v <path to input files>:/input -v <where to create the output dir>:/output nickp60/prophet carton exec /ProphET/ProphET_standalone.pl --fasta_in /input/<name of your input fasta> --gff_in /input/<name of input gff> --outdir /output/<desired name of output folder>
```
